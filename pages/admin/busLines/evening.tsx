...
const EveningList = () => {
  ...
  const busLines = getList(page, search, "evening")
  ...
  useEffect(() => {
    if (onDelete.isError) {
      if (!toast.isActive(toastId5) && get(onDelete, "error.stauts") !== 401) {
        toast({
          id: toastId5,
          title: "Delete",
  ...
  useEffect(() => {
    if (updateRank.isError) {
      if (
        ...
        get(updateRank, "error.stauts") !== 401
      ) {
        toast({
          id: toastId3,
  ...
  useEffect(() => {
    if (busLines.error || busLines.data?.error) {
      if (!toast.isActive(toastId1) && get(busLines, "error.stauts") !== 401) {
        toast({
          id: toastId1,
