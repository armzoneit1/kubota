...
import DummyInput from "./internal/DummyInput"
import ScrollManager from "./internal/ScrollManager"
import RequiredInput from "./internal/RequiredInput"
import { AriaSelection } from "./accessibility/index"
import {
  classNames,
  cleanValue,
  isTouchCapable,
...
>(
  ...
  return props.options
    .map((groupOrOption: any, groupOrOptionIndex) => {
      if ("options" in groupOrOption) {
        const categorizedOptions = groupOrOption.options
          .map((option: any, optionIndex: any) =>
            toCategorizedOption(props, option, selectValue, optionIndex)
          )
          .filter((categorizedOption: any) =>
            isFocusable(props, categorizedOption)
          )
...
>(
  ...
  return alwaysFocusFirstMenuOption
    ? lastFocusedOption && options.indexOf(lastFocusedOption) > -1
      ? lastFocusedOption
      : options[0]
    : null
}
