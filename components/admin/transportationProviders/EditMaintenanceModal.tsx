...
type EditMaintenanceModalProps = {
  ...
  onClose: () => void
  edit: any
  onEdit: (update: any, index: number) => void
}
...
const EditMaintenanceModal = ({
  ...
  useEffect(() => {
    setValue("detail", edit?.data?.detail)
    setValue("mile", edit?.data?.mile)
    setValue("date", new Date(`${edit?.data?.date}`))
    if (edit?.data?.vehicleMaintenanceLogId)
      setValue("vehicleMaintenanceLogId", edit?.data?.vehicleMaintenanceLogId)
    setDate(new Date(`${edit?.data?.date}`))
  }, [isOpen, edit, setValue])
...
export default React.memo(EditMaintenanceModal)
