...
const AreaEdit = ({
  ...
  function onSubmit(values: AreaDataTypes<boolean | string>) {
    ...
    let isEditOrDeleteOrInactiveBusStops: any[] = []
    ...
    const busStops = values.busStops.map((busStop, index) => {
      ...
      let isEditOrDeleteOrInactive = false

      if (!busStop?.status && busStop?.busStopId != null) {
        isEditOrDeleteOrInactive = true
      }

      if (oldData != null) {
        if (busStop?.busStopName != oldData?.busStopName) {
          isEditOrDeleteOrInactive = true
        }
      }

      isEditOrDeleteOrInactiveBusStops.push({
        busStopId: busStop?.busStopId ?? null,
        isEditOrDeleteOrInactive: isEditOrDeleteOrInactive,
        busStopName: busStop?.busStopName,
      })
      busStop.rank = index + 1
      if (!busStop?.busStopId) busStop.busStopId = null
      return busStop
    })

    data?.busStops
      ?.filter((b) => !busStopIds?.includes(b.busStopId))
      ?.forEach((b) => {
        isEditOrDeleteOrInactiveBusStops.push({
    ...
    if (values.status) {
      if (
        isEditOrDeleteOrInactiveBusStops?.find(
          (b) => b.isEditOrDeleteOrInactive
        )
      ) {
        setValues({
          id: data.id,
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        title="ลบพื้นที่"
        content={`หากยืนยันการลบพื้นที่ ${data?.name} รายการจองในอนาคตทั้งหมดของพนักงานที่ลงจุดจอดในพื้นที่นี้จะถูกยกเลิก`}
        type="error"
        acceptLabel="ยืนยัน"
        isOpen={isOpen}
        onClose={onClose}
      ...
      <ConfirmDialog
        title=""
        content="การ inactive พื้นที่ รายการจองในอนาคตทั้งหมดของพนักงานที่ลงจุดจอดในพื้นที่นี้จะถูกยกเลิก"
        onSubmit={() => {
          submit(values)
        }}
        isLoading={isLoading}
      ...
      <ConfirmDialog
        title=""
        content="หากยืนยันการแก้ไขข้อมูลพื้นที่และจุดจอด รายการจองในอนาคตทั้งหมดของพนักงานที่ลงจุดนี้จะถูกยกเลิก"
        onSubmit={() => {
          submit(values)
        }}
        isLoading={isLoading}
